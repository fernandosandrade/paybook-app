...
abstract class ICobrancaService<T extends DocumentRepositoryModel> {
  ...
  Future<T?> findById(String bookId);

  Stream<List<T?>> findByBookId(String bookId);
}

class CobrancaService<T extends DocumentRepositoryModel> implements ICobrancaService<T> {
  ...
  CobrancaService({required this.book, required this.cobranca, required this.serializer})
      : this.collectionPath = RepositoryCollectionPath.of([COBRANCA_COLLECTION, book.wireNumber, cobranca.wireNumber]);

  @override
  Future save(T model) {
    return DocumentRepository.save(DBDocument.fromMap(data: model.toMap(), repositoryCollectionPath: collectionPath));
  }

  @override
  Future delete(T model) {
    return DocumentRepository.delete(DBDocument.fromMap(data: model.toMap(), repositoryCollectionPath: collectionPath));
  }
  ...
  Stream<List<T?>> findByBookId(String bookId) {
    return DocumentRepository.onCollection(this.collectionPath)
        ...
        .map((docList) =>
            docList.isNotEmpty ? docList.map((dbDoc) => dbDoc.toObject(this.serializer)).toList() : List.empty());
  }
